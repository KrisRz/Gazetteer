...
file_put_contents("debug_log.txt", "Hotels API Request - lonMin: $lonMin, latMin: $latMin, lonMax: $lonMax, latMax: $latMax\n", FILE_APPEND);

// OpenTripMap API request URL
$apiKey = "5ae2e3f221c38a28845f05b68a60ac551050a1a74c38664bcc897210";
$url = "https://api.opentripmap.com/0.1/en/places/bbox?lon_min=$lonMin&lat_min=$latMin&lon_max=$lonMax&lat_max=$latMax&kinds=other_hotels&limit=30&format=geojson&apikey=$apiKey";
...
if (!isset($decode['features'])) {
    echo json_encode([
        "status" => [
            "code" => "500",
            "name" => "error",
            "description" => "Invalid API response or no hotels found."
        ],
        "data" => $decode
    ]);
    exit;
